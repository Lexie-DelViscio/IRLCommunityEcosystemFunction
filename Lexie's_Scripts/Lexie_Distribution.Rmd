...
```{r}
comm_data_panels <- with(Long_community_panel, tapply(value, list(Site, Species), as.integer))
comm_data_panels <- ifelse(is.na(comm_data), 0, comm_data)
head(comm_data_panels)
```

```{r}
diversity(comm_data_panels,"simpson")
